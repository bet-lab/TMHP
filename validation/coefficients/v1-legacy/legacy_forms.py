r"""LEGACY (v1) compressor efficiency correlations, frozen verbatim for ablation runs.

This is the module that shipped as ``tmhp.compressor_efficiency`` before the
standalone-compressor refit (validation/compressor_maps).  It is kept here so
the ``legacy-v1`` variant of the EN 14825 trend and parity ablations can be
reproduced; nothing in ``src/`` imports it.

Original module docstring follows.

Default compressor efficiency correlations, and where each one comes from.
