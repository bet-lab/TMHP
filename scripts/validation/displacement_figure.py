...
def main() -> None:
    ...
    point = RATED_POINT_AIR_TO_WATER
    fig, axes = plt.subplots(2, 2, figsize=dm.figsize("17cm", 0.80))

    # --- (a) what the fluid contributes -----------------------------------
    ax = axes[0, 0]
    rho = [volumetric_capacity(r, point.T_evap_C, point.T_cond_C)[0] for r in REFRIGERANTS]
    qvol = [volumetric_capacity(r, point.T_evap_C, point.T_cond_C)[2] / 1e6 for r in REFRIGERANTS]
    x = np.arange(len(REFRIGERANTS))
    ax.bar(x - 0.2, rho, 0.38, color=COLORS["muted"], label=r"suction density $\rho_1$ [kg/m$^3$]")
    ax.bar(x + 0.2, qvol, 0.38, color=COLORS["accent"], label=r"volumetric capacity $\rho_1\Delta h$ [MJ/m$^3$]")
    ax.set_xticks(x)
    ax.set_xticklabels(REFRIGERANTS, fontsize=dm.fs(-2))
    ax.set_ylabel("value at the rating point")
    ax.legend(frameon=False, fontsize=dm.fs(-2), loc="upper right")
    ax.set_title("The fluid does the work", loc="left", fontsize=dm.fs(0))
    ax.grid(True, axis="y", alpha=0.25, linewidth=dm.lw(-2))
    panel_letter(ax, "a")
    ...
    ax.scatter(published, predicted, s=dm.fs(6), c=colors, zorder=4, edgecolor="white", linewidth=dm.lw(-2))
    ax.set_xlim(lo, hi)
    ax.set_ylim(lo, hi)
